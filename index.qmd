...
ggplot(heart, aes(x = factor(HeartDisease))) +
    ...
    xlab("Heart Disease") + ylab("   ")
```

```{r}
h <- hist(heart$Age,ylim = c(0,250),col = "lightgreen",main = "    Age Distribution",xlab = "Age Groups")
text(h$mids,h$counts,labels=h$counts, adj=c(0.5, -0.5))
#lines(density(heart$Age),lwd = 2, col = 'red')
```
...
ggplot(heart, aes(x = factor(ChestPainType))) +
    ...
    theme_classic() +
    ggtitle("                                    Plot of Type of Chest Pain ")+
    xlab("Chest Pain") + ylab("   ")
```
