...
#X = subset(admission,select = -c(chance_of_admit))
...
X_train <- train
#X_train = subset(train,select = -c(chance_of_admit))
#keeps <- c("chance_of_admit")
#y_train = train[keeps]
...
```{r}
gy_logit <- betareg(chance_of_admit ~ serial_no + gre_score + toefl_score + university_rating + sop + lor + cgpa + research, data = X_train)
summary(gy_logit)


```
```{r}
predictions <- predict(gy_logit,X_test)
...
#summarize results
#example <- confusionMatrix(data=predictions, reference = y_test)
#example <- table(predictions,y_test)
